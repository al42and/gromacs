...
public:
    ...
    void makeMtopFromFile(const std::string& fileName, const std::string& mdpContent)
    {
        const std::string simData =
                gmx::test::TestFileManager::getTestSimulationDatabaseDirectory().string();
        ...
        const std::string tprName = fileManager_.getTemporaryFilePath(fileName + ".tpr").string();
        {
            ...
            caller.addOption(
                    "-p",
                    std::filesystem::path(simData).append(fileName).replace_extension(".top").string());
            caller.addOption(
                    "-c",
                    std::filesystem::path(simData).append(fileName).replace_extension(".gro").string());
